...
pop %>% rename( COD_PROVCOM = ï..ITTER107, ABITANTI = Value )%>%
        ...
        mutate(COD_PROVCOM<-as.character(pop$COD_PROVCOM))%>%
        mutate(COD_PROVCOM<-as.numeric(pop$COD_PROVCOM))->pop
...
#considero solo i comuni che fanno parte dei comuniin in ANPR in data 16/04/2020
...
#Non ho la regione di tutti i comuni perchè i dataframe non sono aggiornati alla stesa data. Considero quindi questi comuni come se non rientrassero in ANPR
ANPR <- na.omit(ANPR)
